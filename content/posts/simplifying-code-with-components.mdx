...
We'll start by renaming the class and the file to `EntityMovement` and `PlayerMovement.cs`.
...
A new component is required to implement the interface, `PlayerInputProvider`. This will contain the implementation details that previously lived in `InputProvider`.
...
Attach `PlayerInputProvider` to the player object and ensure the character moves again. A future component could be `EnemyInputProvider` which determines movement based on a `NavMeshAgent` component. 
